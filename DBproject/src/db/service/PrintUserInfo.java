...
public class PrintUserInfo {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("------------------------");
		System.out.print("회원 아이디를 입력하시오 : "); pMap.put("user_id", sc.next());


		System.out.println("------------------------");
		System.out.print("회원 이름를 입력하시오 : "); pMap.put("user_nm", sc.next());
		...
		// rList = ui.getUserInfo(); 함수오류 발생 및 테이블 조회 결과가 없을때 NullPointer 에러가 발생하는것을
		// 방지 하기 위해 강제 메모리 올림
		if (rList == null) {
		...
		while (it.hasNext()) {
			...
			// 예기치 않은 오류로 메모리에 존재하지 않은 경우를 대비하기 위해 강제로 메모리 올림
			if (rMap == null) {
				rMap = new HashMap<String, String>();
			}

			// 레코드의 각 칼럼들 내용 출력
			System.out.println("아이디 : " + rMap.get("user_id"));
			System.out.println("이름 : " + rMap.get("user_nm"));
			System.out.println("이메일 : " + rMap.get("email"));
			System.out.println("주소 : " + rMap.get("addr"));
			System.out.println("가입일 : " + rMap.get("reg_dt"));
			System.out.println("----------------------------");
